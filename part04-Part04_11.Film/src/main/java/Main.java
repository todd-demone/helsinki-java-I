...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        Film bladeRunner = new Film("Blade Runner", 18);
        Scanner input = new Scanner(System.in);
        System.out.println("How old are you?");
        int age = Integer.valueOf(input.nextLine());
        System.out.println("");
        if (age >= bladeRunner.ageRating()) {
            System.out.println("You may watch the film " + bladeRunner.name());
        } else {
            System.out.println("You may not watch the film " + bladeRunner.name());
        }
    }
}
